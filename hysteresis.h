...
int 
SelfCenteringMaterial::setT  (double strain, double strainRate)
{
  
  diffStrain = strain - Cstrain     ;     //Cstrain?????????
  
  if (fabs(diffStrain) < DBL_EPSILON       //fabs 取绝对值
    return 0;                              //如果差值很小的话就取0
  
  // Set total strain
  Tstrain = strain;
  noSlipStrain = Tstrain - CslipStrain;   //无滑动应变=总应变-滑动应变
  
  // Middle Elastic Portion (outside any upper or lower activation)
  //     Entirely elastic response
  if ( fabs(noSlipStrain) <= ( (1-beta) * ActF / k1 ) ) {  //应变在弹性无耗能的阶段
    
    Tstress = k1 * noSlipStrain;   //应力=K1*应变
    Ttangent = k1;                 //此阶段斜率为K1
    
  }
  else {
    
    // Positive Quadrant (Top Right) where strain >= 0     //第一象限内
    if (noSlipStrain >= 0) {
      
      // External Fuse Bolt Bearing
      if (BearDef != 0 && Tstrain > BearDef) {    //Deformation大于bearing strain
	Tstress = BearF + (Tstrain - BearDef) * rBear * k1;    
	Ttangent = rBear * k1;      //post-slip linear bearing
      }
      
      // External Fuse Slip
      else if (SlipDef != 0 && noSlipStrain > SlipDef) {   //出现slip
	Tstress = SlipF;                      //平的阶段
	...
	TactivStrainPos = TupperStrainPos - Tstress / k1;  //开始耗能时的
	...
	TactivStrainPos = TlowerStrainPos - Tstress / k1;    //upper?lower?
